...
public class CraftPlanksTutorialStepMixin {
    ...
    private static MutableComponent modifyTutorialTitle(MutableComponent original) {
        return Main.CONFIG.features.affogatoEarlyGameChanges.get() ? Component.translatable("tutorial.affogato_core.craft_flint_axe.title") : original;
    }
    ...
    private TutorialToast.Icons modifyTutorialIcon(TutorialToast.Icons original) {
        return Main.CONFIG.features.affogatoEarlyGameChanges.get() ? TutorialToast.Icons.valueOf("AFFOGATO_CORE_FLINT_AXE") : original;
    }
    ...
    private static boolean cancel(LocalPlayer player, TagKey<Item> tag, Operation<Boolean> original) {
        return !Main.CONFIG.features.affogatoEarlyGameChanges.get() && original.call(player, tag);
    }
    ...
    private boolean checkFlintAxe(Inventory instance, TagKey<Item> tag, Operation<Boolean> original) {
        return Main.CONFIG.features.affogatoEarlyGameChanges.get() ? instance.contains(itemStack -> itemStack.is(ModItems.FLINT_AXE)) : original.call(instance, tag);
    }
    ...
    private boolean checkFlintAxe(ItemStack instance, TagKey<Item> tagKey, Operation<Boolean> original) {
        return Main.CONFIG.features.affogatoEarlyGameChanges.get() ? instance.is(ModItems.FLINT_AXE) : original.call(instance, tagKey);
    }
}
