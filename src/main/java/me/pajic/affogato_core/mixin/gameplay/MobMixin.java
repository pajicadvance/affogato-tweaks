...
public class MobMixin {
    ...
    private boolean allowAnimalFoodPickup(ItemStack stack, Operation<Boolean> original) {
        if (Main.CONFIG.features.animalsLookForFood.get() && (Mob) (Object) this instanceof Animal animal) {
    ...
    private boolean animalPickupFood(Mob instance, ServerLevel level, ItemEntity itemEntity) {
        if (Main.CONFIG.features.animalsLookForFood.get() && instance instanceof Animal animal && animal.isFood(itemEntity.getItem())) {
            if (!animal.isInLove()) {
    ...
    private boolean preventBadCheck(ItemStack candidate, ItemStack existing, EquipmentSlot slot, Operation<Boolean> original) {
        if (Main.CONFIG.features.animalsLookForFood.get() && (Mob) (Object) this instanceof Animal animal && animal.isFood(candidate)) return false;
