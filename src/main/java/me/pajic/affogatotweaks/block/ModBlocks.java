...
public class ModBlocks {
    ...
    public static final Block EXPOSED_COPPER_LANTERN = new CopperLanternBlock(
            ...
            FabricBlockSettings.create().mapColor(MapColor.TERRACOTTA_LIGHT_GRAY).solid().requiresTool().strength(3.0F).sounds(BlockSoundGroup.LANTERN).luminance(state -> 11).nonOpaque().pistonBehavior(PistonBehavior.DESTROY)
    );

    public static final Block WEATHERED_COPPER_LANTERN = new CopperLanternBlock(
            Oxidizable.OxidationLevel.WEATHERED,
            FabricBlockSettings.create().mapColor(MapColor.DARK_AQUA).solid().requiresTool().strength(3.0F).sounds(BlockSoundGroup.LANTERN).luminance(state -> 7).nonOpaque().pistonBehavior(PistonBehavior.DESTROY)
    );
    public static final Block OXIDIZED_COPPER_LANTERN = new CopperLanternBlock(
            Oxidizable.OxidationLevel.OXIDIZED,
            FabricBlockSettings.create().mapColor(MapColor.TEAL).solid().requiresTool().strength(3.0F).sounds(BlockSoundGroup.LANTERN).luminance(state -> 3).nonOpaque().pistonBehavior(PistonBehavior.DESTROY)
    );
}
